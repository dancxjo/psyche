...
def validate(path: pathlib.Path) -> None:
    ...
    # Optional: validate layer3.services.web.enabled is boolean when present
    web_cfg_layer3 = data.get("layer3", {}).get("services", {}).get("web", {})
    web_enabled = web_cfg_layer3.get("enabled")
    if web_enabled is not None and not isinstance(web_enabled, bool):
        raise ValidationError(
            f"layer3.services.web.enabled must be a boolean in {path}"
        )
